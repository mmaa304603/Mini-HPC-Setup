...
# Function to check if a node is powered off
check_power_state() {
    local node=$1
    local state
...
if check_power_state "$NODE"; then
    log_message "Node $NODE is already powered off"
    exit 0
fi

# Send power off command with retry
retry_count=0
while [ $retry_count -lt $MAX_RETRIES ]; do
    ...
    if [ "$http_code" = "204" ] || [ "$http_code" = "200" ]; then
        log_message "Successfully sent power off command to $NODE"
        
        # Wait for power off
        sleep 10
        if check_power_state "$NODE"; then
            log_message "Node $NODE is now powered off"
            exit 0
        else
            log_message "Warning: Node $NODE may not have powered off properly"
        fi
        break
    fi
    ...
    retry_count=$((retry_count + 1))
    if [ $retry_count -lt $MAX_RETRIES ]; then
        log_message "Retry $retry_count: Failed to power off $NODE (HTTP $http_code)"
        sleep $RETRY_DELAY
    fi
done

if [ $retry_count -eq $MAX_RETRIES ]; then
    log_message "Error: Failed to power off $NODE after $MAX_RETRIES attempts"
    exit 1
fi
