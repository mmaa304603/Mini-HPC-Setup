...
add_unique_image() {
    ...
    for existing in "${warewulf_images[@]}"; do
        [ "$existing" = "$image_name" ] && return 0
    done

    warewulf_images+=("$image_name")
}
...
check_warewulf_image() {
    ...
    if [[ " $HPC_TEST_COMPONENTS " == *" spack "* ]]; then
        if wwctl image exec "$image_name" -- /bin/bash -lc 'test -f /etc/profile.d/spack.sh && findmnt --fstab --mountpoint /opt --types nfs,nfs4 >/dev/null'; then
    ...
    if [[ " $HPC_TEST_COMPONENTS " == *" apptainer "* ]]; then
        if wwctl image exec "$image_name" -- /bin/bash -lc 'command -v apptainer && test -f /etc/apptainer/apptainer.conf && rpm -q squashfs-tools fuse-overlayfs fakeroot'; then
...
check_warewulf_images() {
    local warewulf_images=()
    local image_name

    if [ -n "${HPC_TEST_WAREWULF_IMAGES:-}" ]; then
        for image_name in $HPC_TEST_WAREWULF_IMAGES; do
            add_unique_image "$image_name"
        done
    else
        add_unique_image "${APPTAINER_CPU_IMAGE_NAME:-rockylinux-9.6}"
        add_unique_image "${APPTAINER_GPU_IMAGE_NAME:-}"
    fi

    if [ "${#warewulf_images[@]}" -eq 0 ]; then
        skip "Warewulf image checks disabled; no image names configured"
        return 0
    fi

    for image_name in "${warewulf_images[@]}"; do
        check_warewulf_image "$image_name"
    done
}
